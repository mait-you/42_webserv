...
void Response::errorPage(ServerConfig &srv, LocationConfig *locConfig, codeStatus codeStatus) {
	if (locConfig && locConfig->error_pages.find(codeStatus) != locConfig->error_pages.end())
		handleFile(srv, locConfig, locConfig->error_pages[codeStatus]);
	else if (srv.error_pages.find(codeStatus) != srv.error_pages.end())
		handleFile(srv, locConfig, srv.error_pages[codeStatus]);
	else {
		setHeader("Content-type", "text/html");
		std::string defaultErr = "<html><body style='display:flex;justify-content:center;'><h1>";
		defaultErr += defaultMessage(codeStatus);
