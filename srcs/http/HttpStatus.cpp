...
HttpStatus::Code HttpStatus::getStatusCode() const {
	return _statusCode;
}
...
void HttpStatus::setStatus(Code code, const std::string& message) {
	_statusCode	   = code;
	_statusMessage = message;
}

bool HttpStatus::isError() const {
	return static_cast<int>(_statusCode) >= 400;
}
...
std::string HttpStatus::defaultMessage(Code code) {
	switch (code) {
		case HTTP_200_OK:
			return "OK";
